...
reveal_grid__body:
        ...
        li      $t0, 0                                          # $t0 = row = 0
reveal_grid__loop1:
        bge     $t0, N_ROWS, reveal_grid__epilogue              # if (row >= N_ROWS) goto epilogue 
        li      $t1, 0                                          # $t1 = col = 0
reveal_grid__loop2: 
        bge     $t1, N_COLS, reveal_grid__loop1_end              # if (col >= N_COLS) goto loop1_end
        ...
        lb      $t6, ($t5)                                      # $t6 = grid[row][col]

        li      $t7, IS_RVLD_MASK
        or      $t6, $t6, $t7                                   # grid[row][col] |= IS_RVLD_MASK
        sb      $t6, ($t5)

        addi    $t1, $t1, 1                                     # col = col + 1
        b       reveal_grid__loop2
reveal_grid__loop1_end:
        addi    $t0, $t0, 1                                     # row = row + 1
        b       reveal_grid__loop1
...
place_bombs__loop:
        la      $t1, total_bombs               
        lw      $t2, 0($t1)
        bge     $s2, $t2, place_bombs__epilogue         # if (i >= total_bombs) goto place_bombs__epilogue 
        move    $a0, $s0 
        move    $a1, $s1
...
mark_cell:
        ...
        #
        # Frame:    $ra, $s0, $s1
        # Uses:     $a0, $a1, $v0, $t0, $t1, $t2, $t3, $t4, $t5, $s0, $s1
...
mark_cell__body:
	...
	mul     $t0, $s0, N_COLS                # row * N_COLS
        add     $t1, $t0, $s1                   # row * N_COLS + col
        la      $t2, grid                       
        add     $t3, $t2, $t1                   # $t3 = &grid[row][col]

        lb      $t4, ($t3)                      # $t4 = grid[row][col]
	li      $t0, IS_RVLD_MASK
        and     $t5, $t4, $t0                   # grid[row][col] & IS_RVLD_MASK
	beq     $t5, 0, mark_cell__skip1        # if (grid[row][col] & IS_RVLD_MASK) {
	la      $t1, debug_mode                 #     if (debug_mode) {
        lw      $t2, ($t1)
	beq     $t2, 1, mark_cell__epilogue     #          return;
                                                #     }
	la      $t0, mark_error                 #     printf("Cannot mark a revealed cell.\n");
	move    $a0, $t0
	li      $v0, 4 
	syscall

        b       mark_cell__epilogue             #     return;
                                                # }
mark_cell__skip1:
	li      $t1, IS_MRKD_MASK
	and     $t5, $t4, $t1
	beq     $t5, 0, mark_cell__else1        # if (grid[row][col] & IS_MRKD_MASK) {

	not     $t0, $t1
	and     $t4, $t4, $t0
	sb      $t4, ($t3)                      #   grid[row][col] &= ~IS_MRKD_MASK;

	lw      $t2, bomb_count
	addi    $t2, $t2, 1
        sw      $t2, bomb_count                 #   bomb_count++;

	b       mark_cell__epilogue             # } else {
mark_cell__else1:
        li      $t1, IS_MRKD_MASK
	or      $t4, $t4, $t1                   #   grid[row][col] |= IS_MRKD_MASK
        sb      $t4, ($t3)
...
reveal_cell__body:
        ...
        lb      $t4, ($t3)                      # $t4 = grid[row][col]
	li      $t0, IS_MRKD_MASK
        and     $t5, $t4, $t0                   # grid[row][col] & IS_MRKD_MASK
	beq     $t5, 0, reveal_cell__skip1       # if(grid[row][col] & IS_MRKD_MASK) {

	la      $t1, debug_mode                 #     if (debug_mode) {
        lw      $t2, 0($t1)                     #       return;
	bne     $t2, 0, reveal_cell__epilogue   #     }
				
	la      $t0, reveal_error               # printf("Cannot reveal a marked cell.\n");
	move    $a0, $t0
	li      $v0, 4  
	syscall

        b       reveal_cell__epilogue           # return }

reveal_cell__skip1:
        li      $t0, IS_RVLD_MASK
        and     $t5, $t4, $t0                   # grid[row][col] & IS_RVLD_MASK
        beq     $t5, 0, reveal_cell__skip2       # if(grid[row][col] & IS_RVLD_MASK) {

	la      $t1, debug_mode                 #     if (debug_mode) {
        lw      $t2, 0($t1)                     #       return;
	bne     $t2, 0, reveal_cell__epilogue   #     }

	la      $t0, already_revealed           # printf("Cell is already revealed.\n");
	move    $a0, $t0
	li      $v0, 4 
	syscall
...
reveal_cell__skip2:
        li      $t0, IS_BOMB_MASK
        and     $t5, $t4, $t0                   # grid[row][col] & IS_BOMB_MASK
        beq     $t5, 0, reveal_cell__skip3       # if(grid[row][col] & IS_BOMB_MASK) {

        la      $t2, game_state                 #   game_state = LOSE
        li      $t1, LOSE                       # }
        sw      $t1, ($t2)

reveal_cell__skip3:
        li      $t0, VALUE_MASK
        and     $t5, $t4, $t0                   # grid[row][col] & VALUE_MASK
        bne     $t5, 0, reveal_cell__else1       # if((grid[row][col] & VALUE_MASK) == 0) {

        move    $a0, $s0 
        move    $a1, $s1
        jal     clear_surroundings              #   clear_surroundings(row, col);

        b       reveal_cell__skip4               # }
reveal_cell__else1:
        li      $t1, IS_RVLD_MASK               # else {
	or      $t4, $t4, $t1                   #   grid[row][col] |= IS_RVLD_MASK
        sb      $t4, ($t3) 
        
        la      $t2, game_state                 #   if(game_state != LOSE) {
        lw      $t1, ($t2)
        beq     $t1, LOSE, reveal_cell__skip4

        la      $t5, cells_left                 #       cells_left--;
        lw      $t0, ($t5)                      #   }
	addi    $t0, $t0, -1
        sw      $t0, ($t5)                      # }
...
reveal_cell__skip4:
        la      $t5, cells_left
        lw      $t0, ($t5)
        bne     $t0, 0, reveal_cell__epilogue   # if (cells_left == 0) {

        la      $t2, game_state                 #     game_state = WIN;
        li      $t1, WIN
        sw      $t1, ($t2)                      # }
...
clear_surroundings__body:
        ...
        blt     $s0, 0, clear_surroundings__epilogue
        bge     $s0, N_ROWS, clear_surroundings__epilogue
        blt     $s1, 0, clear_surroundings__epilogue
        bge     $s1, N_COLS, clear_surroundings__epilogue
                                                                #   if (row < 0 || row >= N_ROWS 
                                                                #   || col < 0 || col >= N_COLS) {
                                                                #      return;
                                                                #   }
	...
	li      $t0, IS_RVLD_MASK
        and     $t5, $t4, $t0                                   # grid[row][col] & IS_RVLD_MASK
	bne     $t5, 0, clear_surroundings__epilogue       
                                                                # if(grid[row][col] & IS_RVLD_MASK) {
                                                                #     return;
                                                                # }

	or      $t4, $t4, $t0                                   # grid[row][col] |= IS_RVLD_MASK
        sb      $t4, ($t3)

        la      $t5, cells_left                                 # $t5 = &cells_left
        lw      $t0, ($t5)
	addi    $t0, $t0, -1                                    # cells_left--;
        sw      $t0, ($t5)

        li      $t1, IS_MRKD_MASK
        not     $t0, $t1                                        # grid[row][col] &= ~IS_MRKD_MASK
	and     $t4, $t4, $t0
	sb      $t4, ($t3)

        li      $t0, VALUE_MASK
        and     $t1, $t4, $t0                                   # grid[row][col] & VALUE_MASK
        bne     $t1, 0, clear_surroundings__epilogue
                                                                # if(grid[row][col] & VALUE_MASK) {
                                                                #       return;
                                                                # }        
        addi    $a0, $s0, -1
...
update_highscore__body:
        ...
        move    $s0, $a0                               # $s0 = score
        la      $t0, high_score                        # high_score.score
        lw      $t1, ($t0)

        bge     $t1, $s0, update_highscore__false      # if (high_score.score < score) {

        sw      $s0, ($t0)                             #     high_score.score = score;

        li      $t1, 0                                 #     int i = 0;

update_highscore__loop:
        la      $t2, user_name
        add     $t3, $t1, $t2                          #     $t3 = &user_name[i]
        lb      $t4, ($t3)                             #     $t4 = user_name[i]

        beq     $t4, 0, update_highscore__if_end       #     while (user_name[i] != '\0') {
        
        la      $t0, high_score                        #        high_score
        addi    $t2, $t0, N_INT_BYTES                  #        high_score.name[0]
        add     $t3, $t2, $t1                          #        high_score.name[i]
        sb      $t4, ($t3)                             #        high_score.name[i] = user_name[i];
        addi    $t1, $t1, 1                            #        ++i; 
        
        b       update_highscore__loop                 #     }
update_highscore__if_end:
        li      $t0, 0
        sb      $t0, ($t3)                             # high_score.name[i] = '\0' 

        li      $v0, TRUE                              # return TRUE

        b       update_highscore__epilogue             # }
update_highscore__false:
...
print_scores__body:
        ...
        la      $t0, scores_msg                                 #   printf("-------------SCORES-----------\n\n");
        move    $a0, $t0
        li      $v0, 4
        syscall
        ...
        li      $t1, 0                                          # int i = 0
print_scores__loop:
        bge     $t1, MAX_SCORES, print_scores__body_end         # while(i < MAX_SCORES) {
        mul     $t2, $t1, USER_SCORE_SIZE                       #       USER_SCORE_SIZE * i
        la      $t0, scores                                     #       $t0 = &scores[0]
        add     $t3, $t0, $t2                                   #       $t3 = &scores[i]
        lw      $t4, ($t3)                                      #       struct UserScore curr = scores[i]
                                                                #       curr.score = $t4
        beq     $t4, -1, print_scores__body_end                 #       if (curr.score == -1) {
                                                                #               break;
                                                                #       }
        la      $t0, scores_line_msg                            #       printf("------------------------------\n");
        move    $a0, $t0
        li      $v0, 4
        syscall  

        la      $t0, scores_username_msg                        #       printf("* USERNAME:\t%s\n", curr.name);
        move    $a0, $t0
        li      $v0, 4
        syscall

        addi    $a0, $t3, N_INT_BYTES
        li      $v0, 4
        syscall
        ...
        la      $t0, scores_score_msg                           #       printf("* SCORE:\t%d\n", curr.score);
        move    $a0, $t0
        li      $v0, 4
        syscall
        ...
        addi    $t1, $t1, 1                                     #       i++;
        b       print_scores__loop                              #   }
print_scores__body_end:
        la      $t0, scores_line_msg                            #   printf("------------------------------\n");
        move    $a0, $t0
        li      $v0, 4
        syscall  
