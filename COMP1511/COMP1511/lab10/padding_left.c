...
void pad_left(struct character_node *characters, char pad_character) {
    //add the characters in each node into a string.
    //the string must be the length of the string from string to characters.
    //then place the characters in the string back into the linked list.
    struct character_node *curr = characters;
    char tmp_one = pad_character;
    char tmp_two;
    while (curr != NULL) {
        tmp_two = curr->data;
        curr->data = tmp_one;
