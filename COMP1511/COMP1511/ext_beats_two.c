...
void merge_beats(Track track, int beats_to_merge, int merged_beats) {
    ...
    if (merged_beats > 0 && track->curr != NULL) {
        //if statement only looks at when beats merge into one.
        ...
        while (merge_curr->next != NULL && counter < first_beat_merge) {
            ...
            //current_beat is now track->curr 
            //prev is beat before track->curr
            
            //current is now connected to the track.
            //free the discarded beats
            /*int beat_count = 1;
            while (current_beat != NULL  
                   && beat_count < beats_to_merge) {
                track->curr = track->curr->next;
                prev = current_beat;
...
struct beat *merging_beats(Beat beat, Beat beat_two) {
    ...
    while (a != NULL || b != NULL) {
        ...
        if (lower_note == a) {
            ...
            b = b->next;
        } else if (lower_note == NULL) {
            a = a->next;
            b = b->next;
        } 
    }
    //now a = NULL or b = NULL
    /*while (b == NULL && a != NULL) {
