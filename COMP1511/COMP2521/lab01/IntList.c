...
void IntListInsertInOrder(IntList l, int v) {
	assert(l != NULL);
	// create new int list node, which the specific value v
	struct IntListNode *n = newIntListNode(v);
	// create a current node pointer and a previous node pointer for the while loop
	struct IntListNode *curr = l->first;
	...
	if (l->size == 0) {
		...
		// number is first in the list
		n->next = l->first;
		l->first = n;
	} else if (v >= l->last->data) {
		// number is last in the list
		l->last->next = n;
		l->last = n;
	} else {
		// number is in the middle of the list
...
IntList IntListSortedCopy(IntList l) {
	assert(l != NULL);
	// make new node
	struct IntListRep *new = IntListNew();
	struct IntListNode *curr;
	for (curr = l->first; curr != NULL; curr = curr->next) {
		IntListInsertInOrder(new, curr->data);
	}
	return new;
