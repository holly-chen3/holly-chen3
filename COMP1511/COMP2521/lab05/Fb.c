...
List FbMutualFriends(Fb fb, char *name1, char *name2) {
    ...
    int id1 = nameToId(fb, name1);
    int id2 = nameToId(fb, name2);
    while (count < fb->numPeople) {
        if (fb->friends[count][id1] && fb->friends[count][id2]) {
            ListAppend(l, fb->names[count]);
        }
        count++;
    }
...
void FbFriendRecs1(Fb fb, char *name) {
    // figure out the number of mutual friends someone has with this name
    int *array = malloc(fb->numPeople *sizeof(int));
    int id = nameToId(fb, name);
    int count = 0;
    while (count < fb->numPeople) {
        if (!fb->friends[count][id] && count != id) {
            List l = FbMutualFriends(fb, name, fb->names[count]);
            // show list size
            array[count]  = ListSize(l);
            ListFree(l);
        } else {
            array[count] = 0;
        }
    ...
    int i = fb->numPeople - 2;
    while (i > 0) {
        int index = 0;
        while (index < fb->numPeople) {
            if(array[index] == i) {
