...
Graph GraphMST(Graph g) {
    Graph mst = GraphNew(g->nV); // MST initially empty
    PQ newPQ = PQNew();
    // insert edges into PQ
	for (int i = 0; i < g->nV; i++) {
        for (int j = 0; j < g->nV; j++) {
            ...
            if (weight != 0.0 && i < j) {
                ...
                PQInsert(newPQ, newEdge);
            }
        }
	}
    
    // get all the priority queue edges.
    // if there is a cycle, do not add into the mst edges
    // if there isnt a cycle, add to mst edges
    while ((mst->nE < (g->nV - 1)) || !PQIsEmpty(newPQ)) {
        Edge e = PQExtract(newPQ);
